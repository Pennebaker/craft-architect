...
(function ($) {
    ...
    load(function () {
        ...
        $('#allFilesystems').on('change', function (e) {
            if ($(this).is(':checked')) {
                $('.filesystems [id^="filesystem"]:not(:disabled)').prop('checked', true);
                $('.filesystems [id^="filesystem"]:not(:disabled)').change();
            } else {
                $('.filesystems [id^="filesystem"]:not(:disabled)').prop('checked', false);
                $('.filesystems [id^="filesystem"]:not(:disabled)').change();
            }
        });
        $('.filesystems [id^="filesystem"]:not(:disabled)').on('change', function (e) {
            if ($(this).is(':checked')) {
                if ($('.filesystems [id^="filesystem"]:checked:not(:disabled)').length == $('.filesystems [id^="filesystem"]:not(:disabled)').length) {
                    $('#allFilesystems').prop('checked', true);
                }
            } else {
                $('#allFilesystems').prop('checked', false);
            }
        });

        $('#allImageTransforms').on('change', function (e) {
            if ($(this).is(':checked')) {
                $('.imageTransforms [id^="imageTransform"]:not(:disabled)').prop('checked', true);
                $('.imageTransforms [id^="imageTransform"]:not(:disabled)').change();
            } else {
                $('.imageTransforms [id^="imageTransform"]:not(:disabled)').prop('checked', false);
                $('.imageTransforms [id^="imageTransform"]:not(:disabled)').change();
            }
        });
        $('.imageTransforms [id^="imageTransform"]:not(:disabled)').on('change', function (e) {
            if ($(this).is(':checked')) {
                if ($('.imageTransforms [id^="imageTransform"]:checked:not(:disabled)').length == $('.imageTransforms [id^="imageTransform"]:not(:disabled)').length) {
                    $('#allImageTransforms').prop('checked', true);
                }
            } else {
                $('#allImageTransforms').prop('checked', false);
            }
        });
        ...
        $('[data-filesystem] [type="checkbox"]').on('change', function (e) {
            var parentRow = $(this).closest('[data-filesystem]');
            if ($(this).prop('checked')) {
                var utilizedFilesystem = parentRow.data('filesystem').trim();
                $('.filesystems [data-handle="' + utilizedFilesystem + '"] [type="checkbox"]').prop('checked', true);
                $('.filesystems [data-handle="' + utilizedFilesystem + '"] [type="checkbox"]').change();
            }
        });
        ...
        $('.filesystem[data-handle] [type="checkbox"]').on('change', function (e) {
            if (!isCtrlPressed) {
                var parentRow = $(this).closest('[data-handle]');
                var handle = parentRow.data('handle');
                if (!$(this).prop('checked')) {
                    $('[data-filesystem="' + handle + '"] [type="checkbox"]').prop('checked', false);
                    $('[data-filesystem="' + handle + '"] [type="checkbox"]').change();
                }
            }
        });
